package ru.bogdanov.roman_to_int_13;
...
public class RomanToInt {
    ...
    public int romanToInt(String s) {
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if (curChar == 'I' && i <= chars.length - 2) {
                char nextChar = chars[i + 1];
                if (nextChar == 'V' || nextChar == 'X') {
                    result -= ROMAN.get(curChar);
                } else {
                    result += ROMAN.get(curChar);
                }
            } else if (curChar == 'X' && i <= chars.length - 2) {
                char nextChar = chars[i + 1];
            ...
            } else if (curChar == 'C' && i <= chars.length - 2) {
                char nextChar = chars[i + 1];
